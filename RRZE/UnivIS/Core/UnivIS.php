...
class UnivIS
{
    ...
    protected function ladeMitarbeiterAlle()
    {
        ...
        if (empty($daten)) {
            ...
            if (isset($this->optionen['errormsg']) && $this->optionen['errormsg'] == 1) {
                _e('No employees can be found in this organizational unit.', 'rrze-univis');
            ...
            if ($this->optionen["sortiere_jobs"]) {
                ...
                if (isset($daten['Person']) && isset($this->optionen['sortByLastname']) && $this->optionen['sortByLastname'] == 1) {
                  usort($daten['Person'], function($a, $b) {
		    if ((isset($a['lastname'])) && (isset($b['lastname']))) 
			return $a['lastname'] <=> $b['lastname'];
                  });
                }
                // BK EDIT 05.09.2019 END : added sort by $lastname
